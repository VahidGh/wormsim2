...
| **NFR-QUAL-01** | All C++ source shall pass the project's automated quality checker (`src/cpp/tests/static/cpp_quality_checker.py`, 66 checks in 12 categories), covering: C++20 or later idioms (concepts, ranges, `std::span`, `std::string_view::starts_with`, smart pointers, `std::optional`, `[[nodiscard]]`), RAII resource management (no raw `new`/`delete` outside custom allocators), const-correctness, cache-friendly data layouts (SoA for performance-critical arrays), thread-safety (no mutable global state), and static-analysis cleanliness (`cppcheck --std=c++20` and `clang-tidy` with the project `.clang-tidy` configuration). The checker runs as CTest target `cpp_quality` on every CI push. | Quality | G6, E1, E4 |
