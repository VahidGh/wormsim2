...
| `docs/images/v081_n2_vs_cel_tuned.gif` | v0.8.1 NeuromuscularTuner: 2D bird's-eye N2 vs phase-tuned kinematic traveling wave (114 frames, 10 fps); 48 muscle attachment dots per panel; CEl₄₈=0.01105 BL², RMS=70 µm; A=0.40 rad, λ=1.50 BL, f=0.50 Hz, φ=0.67 rad | Regenerated when tuner parameters or N2 window change |
...
| `fem_body_trace.cpp` | CLI tool (deal.II guard): open-loop sinusoidal D/V muscle activation → FEM body → centerline CSV at 50 Hz; `t_ms,x0,y0,...,x24,y24`; flags `--freq Hz --T_s s --dt_ms ms --amp 0-1` | FEMBody parameter change; new CLI flag; notebook data update |
