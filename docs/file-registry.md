...
| `docs/images/v1105_n2_plate_600s.gif` | v0.11.1 CV-11.5b compact 600 s agar-plate GIF (220 frames @ 20 fps, 11 s, 1.7 MB): full 82.6 BL journey, per-frame real theta_body FK round-trip, head=index 0, Plasma gradient; embedded in notebook + README | Regenerate when WCON data or subsampling changes |
| `docs/images/v1105_n2_plate_600s.html` | v0.11.1 CV-11.5b interactive Plotly agar-plate view full 600 s (616 anim frames ×28 from 17 226 valid WCON frames, bl=0.664 mm, path=82.6 BL); FK panel = real `theta_body` per frame → `compute_skeletons` (round-trip, mean shape error 0.040 BL); head marker (yellow circle) at index 0 (WCON `head='L'`); 12.7 MB | docs/images/ |
