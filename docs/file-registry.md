...
| `docs/install/local_dev_docker.md` | v0.12.2 local dev Docker guide: `Dockerfile.wormsim2-dev` build + test workflow (CI-identical), interactive shell, static analysis, two C++-only literature worked examples (muscle_trace vs Boyle & Cohen 2008; deal.II fem_body_trace vs N2 frequency), no-Docker-GPU-passthrough fallback, block map, macOS notes | install guide |
...
| `docs/images/v1105_n2_plate_view.gif` | v0.11.1 CV-11.5 N2 real vs N2-sim agar-plate view (114 frames = full 3.89 s, 14fps, 547 KB): both worms normalised to 1 BL (real arc 0.656 mm), 0.856 BL forward translocation, Plasma gradient, accumulated trail, cream agar background | notebook CV-11.5 |
| `docs/images/v1105_n2_plate_view.html` | v0.11.1 CV-11.5 interactive Plotly agar-plate view (114 frames = 4 s); play/pause slider; Zenodo option-2 hook in source | notebook CV-11.5 |
| `docs/images/v1105_n2_plate_600s.gif` | v0.11.2 N2 WT agar-plate GIF with head/tail flip correction (221 frames @ 20 fps, 11 s, 578 KB): left=real WCON (flip-corrected), right=4-mode PCA tuner reconstruction (var_exp=0.782, shape err=0.022 BL); generated via `plotly_fig_to_gif()` from `cv115b_tuner_600s.py` | Regenerate when WCON data, flip correction, or tuner parameters change |
...
| `docs/images/local_docker_muscle_trace_cpp.png` | v0.12.2 `muscle_trace` 100/400/700 pA traces, generated live inside `wormsim2-dev` for the AMSC report's C++-only worked example | `docs/install/local_dev_docker.md` §5; regenerate if `muscle_trace` output changes |
| `docs/images/ref_boylecohen2008_muscle_model.png` | v0.12.2 literature reference image for the muscle_trace worked example — `openworm/muscle_model/NeuroML2/C/Plot.png` (Boyle & Cohen 2008 Fig. 2A reference simulation), mirrored locally | `docs/install/local_dev_docker.md` §5; source updated upstream |
| `docs/images/local_docker_fem_body_trace.png` | v0.12.2 `fem_body_trace --freq 0.5` deal.II output (body snapshots, lateral trace, FFT), generated live via the Polimi MK Docker image for the AMSC report's deal.II worked example | `docs/install/local_dev_docker.md` §6; regenerate if `fem_body_trace` output or frequency changes |
| `docs/images/wormsim2_component_diagram.png` | v0.12.2 UML2 component & module diagram (PlantUML), rendered from `docs/design/uml/wormsim2_component_diagram.puml`; used in the AMSC report and `02-architecture-design.md` | Regenerate (see command in `02-architecture-design.md` §1.1) when the C++ static-library chain or Python module set changes |
...
| `docs/design/02-architecture-design.md` | Architecture design — C&C/Module/Deployment views, component interfaces, VR gates, design principles; §1.1 has the as-implemented UML2 component diagram (PlantUML) | New component; interface changed; VR gate added; design principle updated; deployment target added |
| `docs/design/uml/wormsim2_component_diagram.puml` | v0.12.2 PlantUML source for the as-implemented component & module diagram (snake-grid layout); rendered to `docs/images/wormsim2_component_diagram.png` | C++ static-library chain changes; Python module set changes |
